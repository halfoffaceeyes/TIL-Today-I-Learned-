...
## 박스 타입
* Block & Inline
  * Inline Box는 오른쪽 방향으로 흘러가고 Block box는 아래 방향으로 흘러감
* Normal Flow
...
### 속성에 따른 수평 정렬
CSS에서 배치라는건 영역을 옮기는 것이기 때문에 옮기고 싶은 속성에 관점을 두지 말고 그 주변의 영역에 관점을 두는 것이 좋음
![수평 정렬](<이미지/240307/속성에 따른 수평 정렬.PNG>)

## 기타 display 속성
### inline-block
* inline과 block 요소 사이의 중간 지점을 제공하느 display 값
* block 요소의 특징을 가짐
...
* 레이아웃을 변경해야되는 경우에 사용해서 특정 부분만 보이게 만들어주는 역할을 함
...
3. absolute
   ...
   - 아래의 영역의 내용이 많은 경우에 absolute를 뺄 경우 레이아웃이 망가질 수 있음
4. fixed
   - 요소를 Normal Flow에서 제거
...
6. align-items
    * 교착 축을 따라 flex item 행을 정렬(한줄만 정렬됨 두줄이상 정렬하고 싶으면 align-content를 사용)
